...
export default function Transacoes() {
  ...
  function selecionarMes(numeroMes) {
    const [ano] = filtros.mes.split("-");
    const mesFormatado = String(numeroMes).padStart(2, "0");

    setFiltros((atual) => ({ ...atual, mes: `${ano}-${mesFormatado}` }));
    setSeletorMes(false);
  }
  ...
  function mudarMes(delta) {
    setFiltros((atual) => {
      ...
      if (anoCalculado < anoAtual) return atual;

      const totalMesLimite = anoAtual * 12 + mesAtualNumero;
      if (totalMeses > totalMesLimite) return atual;

      const mesFormatado = String(mesCalculado).padStart(2, "0");
      return { ...atual, mes: `${anoCalculado}-${mesFormatado}` };
    });
  }

  const [anoSelecionado, mesSelecionado] = filtros.mes.split("-").map(Number);
        ...
        <div className="relative flex items-center justify-between bg-white rounded-full p-2 mb-3 shadow">
          ...
          >
            ...
            <span className="text-[0.8rem] text-[#2C2438] font-semibold">
              {formatarMesExibicao(filtros.mes)}
            </span>
          ...
          <div className="w-full flex items-center justify-between bg-[#F8F7FB] p-3 rounded-xl mb-4">
            <span className="flex gap-2 items items-center">
              ...
              <div className="leading-4">
                <p className="text-[0.8rem] text-[#2C2438] font-semibold">
